...
public class Process {
	...
	static int EstMota(int nTag, int x, int i, double c){
		...
		for( ; ; ){
			for( ; ; ){
				...
				AboutTag.timer(Tag, nTag, Q);
				TagResponse.Response(Tag, Reader, nTag, i);   //i개만 탐색
			...
			for( ; ; ){			
				...
				AboutTag.timer(Tag, nTag, Q);
				TagResponse.Response(Tag, Reader, nTag, i);
	...
	static int EstModifiedQ(int nTag, int x, int i, double c){
		double Q_Value=x;
		int FinalQ = 0;
		int end = 0;
		int Q = (int) Math.pow(2, x);
		...
		for( ; ; ){
			...
			for( ; ; ){			
				...
				//Ours
				FinalQ = Q+(int)Math.round((double)Q/i*C)-(int)Math.round((double)Q/2/i*I);
	...
	static int EstModifiedCI(int nTag, int x, int i, double c){
		double Q_Value = x;
		int FinalQ = 0;
		int end=0;
		int Q = (int) Math.pow(2, x);
		...
		for( ; ; ){
			for( ; ; ){
				...
				if(I==i){
					...
					if(c==-1) {FinalQ = Q+(int)Math.round((double)Q/i*C)-(int)Math.round((double)Q/2/(i+1)*I); end=1; break;}
					continue;
				}
				basic.RepeatRound += 1;
				basic.time += Time.QueryRepeat(Q, S);
