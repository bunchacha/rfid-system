...
public class Simulation_ModifiedQ{
				...
				for(double c=1 ; c<=1 ; c+=0.2){
					...
					for(int x=5 ; x<=5 ; x++){  
						...
						for(int j=1; j<Si_times+1 ; j++){
							...
							int nextQ = Mota.EstModifiedQ(n0, x, i, c);//Mota 추정
							//data[j][(int)a/100] = nextQ;
							
							int C[] = Mota.firstRound(n0, nextQ);	
						
							for(; C[1] != 0 ;){				
								C = Mota.round1(n0, C[1], C[0]);
								C_round += 1;  
							}
						...
						data[index][(int)a/10] = S_TP/Si_times;	//이거를 Throughput이랑 TimeQ 바꿀 것!
